...
const App: React.FC = () => {
  ...
  // --- RESET LOGIC REMOVED (STABLE STATE) ---
  // The auto-wipe logic has been removed to prevent accidental data deletion.
  // App is now in normal operational mode.
  ...
  const handleValeSave = async (v: Vale) => {
    ...
    if (valeToSave.milling_lot_id) {
      setMillingLots(prevLots => {
        ...
        const newTotalOlives = relevantVales.reduce((acc, v) => acc + v.kilos_netos, 0);
        const newTheoreticalOil = calculateTheoreticalOil(relevantVales);

        // Calcular nueva fecha (mínima de los vales)
        const timestamps = relevantVales.map(v => new Date(v.fecha_entrada).getTime());
        const newDate = timestamps.length > 0 ? new Date(Math.min(...timestamps)).toISOString() : valeToSave.fecha_entrada;

        return prevLots.map(lot => {
          if (lot.id === valeToSave.milling_lot_id) {
            const updatedLot = {
              ...lot,
              kilos_aceituna: newTotalOlives,
              kilos_aceite_esperado: newTheoreticalOil,
              fecha: newDate // Actualizar fecha del lote
            };

            // PERSISTIR CAMBIOS EN EL LOTE (CRÍTICO)
            upsertMillingLot(updatedLot).catch(err => console.error("Error actualizando Lote MT al guardar Vale:", err));
