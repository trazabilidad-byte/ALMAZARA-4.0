...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, currentUser, navItems }) => {
      ...
      <nav className="flex-1 flex flex-col gap-1 md:gap-2 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
        {itemsToRender.map((item) => (
          <button
            ...
            className={`flex items-center gap-4 p-3 rounded-xl transition-all relative group shrink-0 ${
              activeTab === item.id 
      ...
      <div className="mt-auto pt-4 border-t border-gray-800 shrink-0">
        <div className="flex items-center gap-3 px-1 lg:px-2">
          ...
          <div className="hidden lg:block overflow-hidden">
            <p className="text-sm font-semibold truncate">{currentUser?.fullName ? String(currentUser.fullName) : 'Administrador'}</p>
            <p className="text-xs text-gray-500 uppercase truncate">Uso Privado</p>
          </div>
        </div>
      </div>
    </div>
