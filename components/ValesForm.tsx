...
import { Producer, OliveVariety, ValeType, Vale, ValeStatus, Hopper, Customer, CustomerType, MillingLot, Tank, PackagingLot, OilExit, ExitType, ProductionLot } from '../types';
...
export const ValesForm: React.FC<ValesFormProps> = ({ 
  ...
  millingLots = [], productionLots = [], tanks = [], packagingLots = [], oilExits = [],
  ...
  const traceInfo = useMemo(() => {
      if (!editVale || isDirectSale) return null;

      // 1. MOLIENDA (Lote MT)
      const millingBatch = editVale.milling_lot_id 
          ? millingLots.find(m => m.id === editVale.milling_lot_id) 
          : null;

      // 2. PRODUCCIÓN (Lote LP - Tanda)
      const productionBatch = millingBatch 
          ? productionLots.find(lp => lp.millingLotsIds.includes(millingBatch.id)) 
          : null;

      // 3. BODEGA (Depósito)
      // Si tenemos tanda cerrada, el depósito destino está en el LP, si no, intentamos sacarlo del MT
      const tankId = productionBatch ? productionBatch.targetTankId : (millingBatch ? millingBatch.deposito_id : null);
      ...
      if (tank) {
          // Si el tanque está vacío o muy bajo, asumimos que salió
          if (tank.currentKg < 100 && millingBatch) {
              // Buscar movimientos de salida posteriores a la fecha de molturación
              const batchDate = new Date(millingBatch.fecha);
              
              // Buscar en Envasado
              const packLot = packagingLots.find(p => {
                  const pDate = new Date(p.date);
                  return pDate >= batchDate && (p.sourceInfo.includes(`D${tank.id}`) || p.sourceInfo.includes(`D.${tank.id}`));
              });

              // Buscar en Granel
              const bulkExit = oilExits.find(e => {
              ...
              if (packLot) {
                  ...
                  exitType = 'UNKNOWN_EXIT'; // Posible trasiego no rastreado aquí
              }
          }
      }

      return { millingBatch, productionBatch, tank, allocatedOilKg, contributionPercent, exitType, exitInfo };
  }, [editVale, millingLots, productionLots, tanks, packagingLots, oilExits, isDirectSale]);
  ...
  return (
    <div className="bg-white rounded-[32px] md:rounded-[40px] custom-shadow p-5 md:p-8 max-w-5xl mx-auto border border-gray-100 animate-in zoom-in duration-300 w-full overflow-hidden relative">
                    ...
                    <div className="flex flex-col gap-3">
                       ...
                       <div className="flex gap-2 flex-wrap">
                           ...
                           <button 
                               ...
                               className={`w-12 h-12 rounded-full flex items-center justify-center font-black text-lg transition-all border-2 ${
                                   formData.ubicacion_id === hopper.id 
      ...
      {editVale && !isDirectSale && traceInfo && (
          <div className="mt-8 border-t border-gray-100 pt-8 animate-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  {/* FASE 4: SALIDA */}
                  <div className={`p-6 rounded-[32px] border transition-all ${traceInfo.exitType !== 'STOCK' ? 'bg-[#111111] text-white shadow-xl' : 'bg-white border-gray-100'}`}>
                      <div className="flex items-center gap-2 mb-4 text-[#D9FF66]">
                          {traceInfo.exitType === 'PACKAGING' ? <Package size={18} /> : <Truck size={18} />}
                          <span className="text-xs font-black uppercase">Salida / Venta</span>
                      </div>
                      {traceInfo.exitType !== 'STOCK' && traceInfo.exitInfo ? (
                          <div>
                              <p className="text-2xl font-black uppercase mb-1 text-white">
                                  {traceInfo.exitType === 'PACKAGING' ? traceInfo.exitInfo.id : traceInfo.exitInfo.deliveryNote || 'Salida Granel'}
                              </p>
                              <p className="text-[10px] text-gray-400 font-bold uppercase">{new Date(traceInfo.exitInfo.date).toLocaleDateString()}</p>
                              <div className="mt-4">
                                  <span className="px-3 py-1 bg-[#D9FF66] text-black rounded-lg text-[9px] font-black uppercase">
                                      {traceInfo.exitType === 'PACKAGING' ? 'Envasado' : 'Venta Cisterna'}
                                  </span>
                              </div>
                          </div>
                      ) : (
                          <div className="text-center py-6">
                              <p className="text-sm font-black text-green-900 uppercase bg-green-50 px-3 py-1 rounded-lg inline-block border border-green-200">En stock (Depósito)</p>
                          </div>
                      )}
                  </div>
