...
export const PackagingDashboard: React.FC<PackagingDashboardProps> = ({
  ...
  // Filtrar lotes disponibles según formato seleccionado (USANDO LIVE STOCK)
  const filteredAuxLots = useMemo(() => {
      const selectedFormat = packagingFormats.find(f => f.name === currentLine.format);
      const formatNameSimple = currentLine.format ? currentLine.format.toLowerCase().replace('botella', '').trim() : '';

      return {
          bottles: liveAuxLots.filter(l => {
              if (l.category !== 'Envase') return false;
              const nameLower = String(l.materialType).toLowerCase();
              return formatNameSimple ? nameLower.includes(formatNameSimple) : true;
          }),
          caps: liveAuxLots.filter(l => l.category === 'Tapón'),
          labels: liveAuxLots.filter(l => l.category === 'Etiqueta')
      };
  }, [liveAuxLots, currentLine.format, packagingFormats]);
            ...
            {packagingLines.length > 0 && (
                <div className="bg-gray-50 rounded-2xl p-4 mb-6 border border-gray-200">
                    ...
                    <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-200">
                        <span className="text-xs font-black text-gray-500 uppercase">Total Estimado Aceite (Litros)</span>
                        <span className="text-2xl font-black text-[#111111]">{Math.round(orderTotals.requiredLiters).toLocaleString()} L</span>
                    </div>
                </div>
            )}
