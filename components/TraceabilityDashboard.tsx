...
import { PackagingLot, MillingLot, Vale, Producer, Customer, SalesOrder, Tank, AppConfig, OilMovement, OilExit, ExitType, ProductionLot } from '../types';
...
export const TraceabilityDashboard: React.FC<TraceabilityDashboardProps> = ({
  ...
  const runSearch = (term: string) => {
    ...
    if (packLot) {
        ...
        let foundMillingLot = millingLots.find(mt => mt.deposito_id === tankId) || millingLots[0]; 
         ...
         <div className="space-y-12 max-w-[1600px] mx-auto px-4">
            <div className="bg-[#111111] text-white p-8 rounded-[40px] shadow-2xl relative overflow-hidden">
               <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                  ...
                  <button className="bg-[#D9FF66] text-black px-6 py-3 rounded-xl font-black uppercase text-xs tracking-widest hover:scale-105 transition-all flex items-center gap-2"><FileDown size={18} /> Descargar PDF</button>
               </div>
            </div>
